...
// Check slider status.
add_filter( 'business_field_filter_slider_status', 'business_field_check_slider_status' );

// Add slider to the theme.
add_action( 'business_field_action_before_content', 'business_field_add_featured_slider', 5 );

// Slider details.
add_filter( 'business_field_filter_slider_details', 'business_field_get_slider_details' );

if ( ! function_exists( 'business_field_get_slider_details' ) ) :
	/**
	 * Slider details.
	 *
	 * @since 1.0.0
	 ...
	 */
	function business_field_get_slider_details( $input ) {

		$featured_slider_type           = business_field_get_option( 'featured_slider_type' );
		$featured_slider_number         = business_field_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = business_field_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = business_field_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$item['id'] = absint( $id );
						$item['caption_alignment'] = business_field_get_option( 'featured_slider_page_caption_alignment_' . $i );
						$blocks[ $item['id'] ] = $item;
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$image_array = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'business-field-slider' );
							$slides[ $cnt ]['images']  = $image_array;
							$slides[ $cnt ]['title']   = esc_html( $post->post_title );
							$slides[ $cnt ]['url']     = esc_url( get_permalink( $post->ID ) );
							$slides[ $cnt ]['excerpt'] = business_field_the_excerpt( apply_filters( 'business_field_filter_slider_caption_length', 30 ), $post );
...
if ( ! function_exists( 'business_field_add_featured_slider' ) ) :
	/**
	 * Add featured slider.
	 *
	 * @since 1.0.0
	 */
	function business_field_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'business_field_filter_slider_status', false );
		if ( true !== $flag_apply_slider ) {
			return false;
		}

		$slider_details = array();
		$slider_details = apply_filters( 'business_field_filter_slider_details', $slider_details );
		...
		// Render slider now.
		business_field_render_featured_slider( $slider_details );

	}
endif;

if ( ! function_exists( 'business_field_render_featured_slider' ) ) :
	/**
...
if( ! function_exists( 'business_field_check_slider_status' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function business_field_check_slider_status( $input ) {

		// Slider status.
		$featured_slider_status = business_field_get_option( 'featured_slider_status' );
